...
export function intersectsOBB3(a: OBB3, b: OBB3, epsilon = Number.EPSILON): boolean {
    ...
    // |R| + epsilon (epsilon guards against near-parallel edges giving a zero cross)
    const q00 = Math.abs(r00) + epsilon;
    const q01 = Math.abs(r01) + epsilon;
    const q02 = Math.abs(r02) + epsilon;
    const q10 = Math.abs(r10) + epsilon;
    const q11 = Math.abs(r11) + epsilon;
    const q12 = Math.abs(r12) + epsilon;
    const q20 = Math.abs(r20) + epsilon;
    const q21 = Math.abs(r21) + epsilon;
    const q22 = Math.abs(r22) + epsilon;
