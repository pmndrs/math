...
group('raycast closest-hit 64x1024 @algo @raycast', () => {
  bench('rays vs triangle soup', function* () {
    ...
    for (let i = 0; i < TRIANGLES; i++) {
      ...
      const a: Vec3 = [cx + (mulberry32.sample(rand) - 0.5) * 4, cy + (mulberry32.sample(rand) - 0.5) * 4, cz + (mulberry32.sample(rand) - 0.5) * 4];
      const b: Vec3 = [cx + (mulberry32.sample(rand) - 0.5) * 4, cy + (mulberry32.sample(rand) - 0.5) * 4, cz + (mulberry32.sample(rand) - 0.5) * 4];
      const c: Vec3 = [cx + (mulberry32.sample(rand) - 0.5) * 4, cy + (mulberry32.sample(rand) - 0.5) * 4, cz + (mulberry32.sample(rand) - 0.5) * 4];
      triA.push(a);
      triB.push(b);
      triC.push(c);
    ...
    for (let i = 0; i < RAYS; i++) {
      ...
      const origin: Vec3 = [Math.cos(theta) * 25, (mulberry32.sample(rand) - 0.5) * 10, Math.sin(theta) * 25];
      const target: Vec3 = [
    ...
    yield () => {
      ...
      for (let r = 0; r < RAYS; r++) {
        ...
        for (let t = 0; t < TRIANGLES; t++) {
          if (!raycast3.intersectsBox3(origin, direction, RAY_LENGTH, aabbs[t])) continue;
          raycast3.intersectsTriangle(result, origin, direction, RAY_LENGTH, triA[t], triB[t], triC[t], false);
        ...
        if (bestFraction < Infinity) {
          ...
          vec3.scaleAndAdd(hitPoint, origin, direction, bestFraction * RAY_LENGTH);
        }
      }
      
      sink = fractionSum + hits;
    };
  }).gc('inner');
});
